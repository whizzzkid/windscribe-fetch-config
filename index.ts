...
import {
  ConfigFetcher,
  WindscribeConfigWorker,
} from './WindscribeConfigWorker';
...
const progressBar = new cliProgress.SingleBar(
  {
    format:
      'Fetching From Windscribe: {percentage}% - {bar} - {value}/{total} Locations',
    barsize: 50,
  },
...
const workers = new Array(workerCount)
  ...
  .map(
    (): WindscribeConfigWorker =>
      new WindscribeConfigWorker(locations, progressBar, fetchers),
  );

await Promise.all(
  workers.map((worker: WindscribeConfigWorker): Promise<void> => worker.run()),
);
